    ...
    <div x-data="{ sidebarOpen: false }" style="margin-top:-1px;" class="flex h-screen overflow-hidden">
        <aside
            ...
            <nav class="flex flex-col gap-4">
                ...
                <a class=" {{ request()->routeIs('containers.show') ? 'bg-red-400 font-bold' : 'hover:bg-yellow-400' }} text-white text-xl text-center hover:bg-yellow-400" href="{{ route('containers.index') }}">Container</a>
        ...
        <div class="min-h-screen w-full bg-gray-100 flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-3xl w-full bg-white shadow-xl rounded-2xl p-8 space-y-6  animate-fadeIn">
                <h2 class="text-2xl font-bold text-indigo-600 text-center">Détails du Container</h2>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 text-gray-700">
                    <div>
                        <span class="font-semibold">Numéro :</span>
                        <p class="text-black-900">{{ $container->numerocontainer }}</p>
                    </div>

                    <div>
                        <span class="font-semibold">Taille :</span>
                        <p class="text-black-900">{{ $container->taillecontainer }}</p>
                    </div>

                    <div>
                        <span class="font-semibold">Type de Cargaison :</span>
                        <p class="text-black-900">{{ $container->typecargaison }}</p>
                    </div>

                    <div>
                        <span class="font-semibold">Pays d'origine :</span>
                        <p class="text-black-900">{{ $container->paysorigine }}</p>
                    </div>

                    <div>
                        <span class="font-semibold">Destination :</span>
                        <p class="text-black-900">{{ $container->destination }}</p>
                    </div>

                    <div>
                        <span class="font-semibold">Poids :</span>
                        <p class="text-black-900">{{ $container->poidscontainer }} kg</p>
                    </div>

                    <div>
                        <span class="font-semibold">Date d'arrivée :</span>
                        <p class="text-black-900">{{ \Carbon\Carbon::parse($container->datearrivee)->format('d/m/Y') }}</p>
                    </div>

                    <div>
                        <span class="font-semibold">État de l'inspection :</span>
                        <p class="text-black-900">{{ $container->etatinspection }}</p>
                    </div>

                    <div class="sm:col-span-2">
                        <span class="font-semibold">Nom de la cargaison :</span>
                        <p class="text-black-900">{{ $container->cargaison->nomcargaison ?? 'N/A' }}</p>
                    </div>
                </div>

                <div class="text-center mt-6">
                    <a href="{{ route('containers.index') }}" class="inline-block bg-indigo-700 hover:bg-green-700 text-white font-semibold py-2 px-6 rounded-lg transition">
                        Retour à la liste
                    </a>
                </div>
            </div>
