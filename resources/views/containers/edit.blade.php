    ...
    <div x-data="{ sidebarOpen: false }" style="margin-top:-1px;" class="flex h-screen overflow-hidden">
        <aside
            ...
            <nav class="flex flex-col gap-4">
                ...
                <a class=" {{ request()->routeIs('containers.edit') ? 'bg-red-400 font-bold' : 'hover:bg-yellow-400' }} text-white text-xl text-center hover:bg-yellow-400" href="{{ route('containers.index') }}">Container</a>
        ...
        <div class="min-h-screen w-full bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-2xl mx-auto bg-white shadow-xl rounded-2xl p-8 space-y-6  animate-fadeIn">
                ...
                <form id="containerForm" action="{{ isset($container) ? route('containers.update', $container->idcontainer) : route('containers.store') }}" method="POST" class="space-y-4">
                    @csrf
                    @if(isset($container))
                    @method('PUT')
                    @endif
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="typecargaison" id="typecargaison" class="w-full border px-3 py-2 rounded" required>
                                <option value="">-- Choisir le type --</option>
                                @foreach(['uniforme', 'mixte', 'liquide','dangereux','réfrigerer'] as $type)
                                <option value="{{ $type }}" {{ old('typecargaison', $container->typecargaison ?? '') == $type ? 'selected' : '' }}>
                                    {{ ucfirst($type) }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="etatinspection" id="etatinspection" class="w-full border px-3 py-2 rounded" required>
                                ...
                                <option value="{{ $etat }}" {{ old('etatinspection', $container->etatinspection ?? '') == $etat ? 'selected' : '' }}>
                                    {{ ucfirst($etat) }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="text-center mt-6">
                            <button type="submit"
                                class="bg-indigo-600 hover:bg-green-700 text-white font-semibold py-2 px-6 rounded-lg transition" id="updateButton">
                                {{ isset($container) ? 'Mettre à jour' : 'Enregistrer' }}
                            </button>
                            <a href="{{ route('containers.index') }}"
                                class="font-semibold py-2 px-6 rounded-lg  text-white transition bg-red-700 hover:bg-gray-600" id="cancelBtn">Annuler</a>
                        </div>
                </form>
            </div>
        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                updateButton.addEventListener('click', function(e) {
                    if (!confirm('Êtes-vous sûr de vouloir mettre à jour ce container ?')) {
                        e.preventDefault();
                    }
                });
            });
