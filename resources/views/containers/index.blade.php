<x-app-layout>
    <div x-data="{ sidebarOpen: false }" style="margin-top:-1px;" class="flex h-screen overflow-hidden">
        ...
        <div class="min-h-screen w-full bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl w-4/5 mx-auto">
                <div class="flex  px-4 py-4  justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-indigo-600">Liste des Containers</h1>
                ...
                <div class="overflow-x-auto w-4/5bg-white shadow rounded-2xl">
                    <table class="min-w-full  divide-y divide-gray-200 text-sm text-left" id="containersTable">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white text-center">
                            ...
                            <tr class="hover:bg-gray-200 transition">
                                <td class="px-6 py-4">{{ $container->numerocontainer }}</td>
                                <td class="px-6 py-4">{{ $container->paysorigine }}</td>
                                <td class="px-6 py-4">{{ $container->destination }}</td>
                                <td class="px-6 py-4">
                                    {{ $container->cargaison?->nomcargaison ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 flex gap-2 justify-center">
                                    <a href="{{ route('containers.show', $container->idcontainer) }}"
                                        class="bg-blue-600 hover:bg-green-700 text-white text-center px-3 py-1 rounded text-xs">
                                        Détails
                                    </a>
                                    <a href="{{ route('containers.edit', $container->idcontainer) }}"
                                        class="bg-orange-500 hover:bg-yellow-600 text-white text-center px-3 py-1 rounded text-xs">
                                        Modifier
                                    </a>
                                    <form action="{{ route('containers.destroy', $container->idcontainer) }}" method="POST" class="inline delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
        ...
        <script>
            ...
            const table = document.getElementById('containersTable').getElementsByTagName('tbody')[0];
