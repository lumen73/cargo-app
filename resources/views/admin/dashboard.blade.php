        ...
        <div class="flex-1 flex flex-col w-full ml-0 md:ml-64">
            ...
            <main class="p-6">
                <div class="flex justify-center mb-6">
                    <h1 class="text-3xl font-bold text-center w-full">Liste des utilisateurs</h1>
                </div>
                    ...
                    <table class="w-2/3 table-auto border-collapse border border-green-500">
                        <thead class="bg-blue-600 text-white text-center">
                            <tr>
                                ...
                                <th class="border px-3 py-3">Nom</th>
                                <th class="border px-3 py-3">Email</th>
                                <th class="border px-3 py-3">Rôle</th>
                                <th class="border px-3 py-3">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr class="text-center">
                                <td class="border px-3 py-3">{{ $loop->iteration }}</td>
                                <td class="border px-3 py-3">{{ $user->name }}</td>
                                <td class="border px-3 py-3">{{ $user->email }}</td>
                                ...
                                <td class="flex py-3">
                                    <a style="margin-left: 10px" href="{{ route('admin.show1', $user->id) }}" class="bg-green-600 hover:bg-gray-600 text-white px-4 py-2 rounded-lg border border-green-500 hover:border-green-600">Afficher</a>
                                    <a style="margin-left: 10px" href="{{ route('admin.edit', $user->id) }}" class="bg-orange-600 hover:bg-white text-black px-4 py-2 rounded-lg border border-orange-400 hover:border-orange-500">Modifier</a>
                                    <form style="margin-left: 10px; margin-right: 10px" action="{{ route('users.destroy', $user->id) }}" method="POST" class="flex justify-center gap-4 rounded-lg border border-red-500 flex-wrap delete-user-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-600 px-4 py-2 text-white  rounded-lg border hover:bg-gray-600 show-confirm">supprimer</button>
                                    </form>
