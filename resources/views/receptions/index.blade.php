<x-app-layout>
    <div x-data="{ sidebarOpen: false }" style="margin-top:-1px;" class="flex h-screen overflow-hidden">
        ...
        <div class="min-h-screen w-full bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl w-4/5 mx-auto">
                <div class="flex px-4 py-4 justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-indigo-600">Liste des Réceptions</h1>
                    <a href="{{ route('receptions.create') }}" class="bg-indigo-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg shadow transition">+ Nouvelle Réception</a>
                </div>
                ...
                <div class="flex justify-center items-center mb-6">
                    <div class="container">
                        <div class="flex justify-end">
                            <input type="text" id="searchInput" placeholder="Rechercher..." class="px-4 py-2 border rounded shadow w-1/3 focus:outline-none focus:ring focus:border-blue-300">
                        </div>
                    </div>
                    <button type="submit" class="bg-indigo-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded ml-2">
                        Rechercher
                    </button>
                </div>

                <div class="overflow-x-auto w-full bg-white shadow rounded-2xl">
                    <table class="min-w-full divide-y divide-gray-200 text-sm text-left" id="receptionsTable">
                        <thead class="bg-cyan-500 text-white text-center text-sm font-semibold">
                            <tr>
                                ...
                                <th class="px-6 py-3">Date d'arrivée</th>
                                <th class="px-6 py-3">Lieu de Réception</th>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white text-center">
                            ...
                            <tr class="hover:bg-gray-200 transition">
                                <td class="px-6 py-4">{{ $r->idreception }}</td>
                                <td class="px-6 py-4">{{ $r->cargaison->nomcargaison ?? 'N/A' }}</td>
                                <td class="px-6 py-4">{{ $r->datearrivee }}</td>
                                <td class="px-6 py-4">{{ $r->lieudereception }}</td>
                                <td class="px-6 py-4 flex gap-2 justify-center">
                                    <a href="{{ route('receptions.show', $r->idreception) }}" class="bg-blue-600 hover:bg-green-700 text-white px-3 py-1 rounded text-xs">Détails</a>
                                    <a href="{{ route('receptions.edit', $r->idreception) }}" class="bg-orange-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-xs">Modifier</a>
                                    <form action="{{ route('receptions.destroy', $r->idreception) }}" method="POST" class="inline delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-600 hover:bg-red-800 text-white px-3 py-1 rounded text-xs">Supprimer</button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
        ...
        <script>
            ...
            const table = document.getElementById('receptionsTable').getElementsByTagName('tbody')[0];
