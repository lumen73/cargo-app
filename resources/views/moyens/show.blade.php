    ...
    <div x-data="{ sidebarOpen: false }" style="margin-top:-1px;" class="flex h-screen overflow-hidden">
        <aside
            ...
            <nav class="flex flex-col gap-4">
                ...
                <a class="{{ request()->routeIs('moyens.show') ? 'bg-red-400 font-bold' : 'hover:bg-yellow-400' }} text-white text-xl text-center hover:bg-yellow-400" href="{{ route('moyens.index') }}">Moyen</a>
        ...
        <div class="min-h-screen w-full bg-gray-100 flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-3xl w-full bg-white shadow-xl rounded-2xl p-8 space-y-6 animate-fadeIn">
                <h2 class="text-2xl font-bold text-indigo-600 text-center">Détails du Moyen</h2>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 text-gray-700">
                    <div>
                        <span class="font-semibold">Transport :</span>
                        <p class="text-black">{{ $moyen->transport }}</p>
                    </div>
                    <div>
                        <span class="font-semibold">Nom :</span>
                        <p class="text-black">{{ $moyen->nomchauffeur }}</p>
                    </div>
                    <div>
                        <span class="font-semibold">Prénom :</span>
                        <p class="text-black">{{ $moyen->prenomschauffeur }}</p>
                    </div>
                    <div>
                        <span class="font-semibold">Numéro :</span>
                        <p class="text-black">{{ $moyen->numero }}</p>
                    </div>
                    <div>
                        <span class="font-semibold">Permis :</span>
                        <p class="text-black">{{ $moyen->permis }}</p>
                    </div>
                </div>

                <div class="text-center mt-6">
                    <a href="{{ route('moyens.index') }}" class="inline-block bg-indigo-700 hover:bg-green-700 text-white font-semibold py-2 px-6 rounded-lg transition">
                        ← Retour à la liste
                    </a>
                </div>
            </div>
