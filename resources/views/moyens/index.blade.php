<x-app-layout>
    <div x-data="{ sidebarOpen: false }" style="margin-top:-1px;" class="flex h-screen overflow-hidden">
        ...
        <div class="min-h-screen w-full bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl w-4/5 mx-auto">
                <div class="flex px-4 py-4 justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-indigo-600">Liste des Moyens</h1>
                    <a href="{{ route('moyens.create') }}"
                ...
                <div class="overflow-x-auto w-full bg-white shadow rounded-2xl">
                    <table class="min-w-full divide-y divide-gray-200 text-sm text-left" id="moyensTable">
                        <thead class="bg-cyan-500 text-white text-center text-sm font-semibold">
                            <tr>
                                <th class="px-6 py-3">Transport</th>
                                <th class="px-6 py-3">Nom chauffeur</th>
                                <th class="px-6 py-3">Prénom chauffeur</th>
                                <th class="px-6 py-3">Numéro</th>
                                <th class="px-6 py-3">Permis</th>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white text-center">
                            ...
                            <tr class="hover:bg-gray-200 transition">
                                <td class="px-6 py-4">{{ $moyen->transport }}</td>
                                <td class="px-6 py-4">{{ $moyen->nomchauffeur }}</td>
                                <td class="px-6 py-4">{{ $moyen->prenomschauffeur }}</td>
                                <td class="px-6 py-4">{{ $moyen->numero }}</td>
                                <td class="px-6 py-4">{{ $moyen->permis }}</td>
                                <td class="px-6 py-4 flex gap-2 justify-center">
                                    <a href="{{ route('moyens.show', $moyen) }}"
                                        class="bg-blue-600 hover:bg-green-700 text-white text-center px-3 py-1 rounded text-xs">
                                        Détails
                                    </a>
                                    <a href="{{ route('moyens.edit', $moyen) }}"
                                        class="bg-orange-500 hover:bg-yellow-600 text-white text-center px-3 py-1 rounded text-xs">
                                        Modifier
                                    </a>
                                    <form action="{{ route('moyens.destroy', $moyen) }}" method="POST" class="inline delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                            ...
                            @empty
                            <tr>
                                <td colspan="6" class="text-center py-6 text-gray-500 italic">Aucun moyen disponible.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="mt-4">
                    {{ $moyens->appends(request()->query())->links() }}
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            const table = document.getElementById('moyensTable').getElementsByTagName('tbody')[0];
