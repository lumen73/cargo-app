<x-app-layout>
    <div x-data="{ sidebarOpen: false }" style="margin-top:-1px;" class="flex h-screen overflow-hidden">
        ...
        <div class="min-h-screen w-full bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl w-4/5 mx-auto">
                <div class="flex px-4 py-4 justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-indigo-600">Liste des Zones</h1>
                    <a href="{{ route('zones.create') }}" class="bg-indigo-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg shadow transition">+ Nouvelle Zone</a>
                </div>
                ...
                <div class="overflow-x-auto w-full bg-white shadow rounded-2xl">
                    <table class="min-w-full divide-y divide-gray-200 text-sm text-left" id="zonesTable">
                        <thead class="bg-cyan-500 text-white text-center text-sm font-semibold">
                            <tr>
                                ...
                                <th class="px-6 py-3">Zone de Stockage</th>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white text-center">
                            ...
                            <tr class="hover:bg-gray-200 transition">
                                <td class="px-6 py-4">{{ $z->container?->cargaison?->nomcargaison ?? 'N/A' }}</td>
                                <td class="px-6 py-4">{{ $z->container?->numerocontainer ?? 'N/A' }}</td>
                                <td class="px-6 py-4">{{ $z->zonestockage }}</td>
                                <td class="px-6 py-4">{{ $z->datestockage }}</td>
                                <td class="px-6 py-4">{{ $z->heurestockage }}</td>
                                <td class="px-6 py-4 flex gap-2 justify-center">
                                    <a href="{{ route('zones.show', $z->idzone) }}" class="bg-blue-600 hover:bg-green-700 text-white px-3 py-1 rounded text-xs">Détails</a>
                                    <a href="{{ route('zones.edit', $z->idzone) }}" class="bg-orange-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-xs">Modifier</a>
                                    <form action="{{ route('zones.destroy', $z->idzone) }}" method="POST" class="inline delete-form">
                                        @csrf
                                        @method('DELETE')
        ...
        <script>
            ...
            const table = document.getElementById('zonesTable').getElementsByTagName('tbody')[0];
